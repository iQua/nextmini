...
async fn test_node_server_start_listening(){
    ...
    let handle = tokio::spawn(async move {
        node_server.start_listening(&"127.0.0.1:9861".to_string()).await;
    });
    ...
    let mut stream = tokio::net::TcpStream::connect("127.0.0.1:9861").await.expect("Failed to connect to server");

    //Write session id to the fake tcp node stream
    stream.write(&session_id).await.expect("Failed to send session id");

    let node_id: u64 = 1;
    stream.write(&node_id.to_be_bytes()).await.expect("Failed to send node id");
    ...
    stream.write(&dummy_ip_frame[..]).await.expect("Failed to write to server");
    ...
    assert!(packet.packet_size == 60, "Expected 60, but got {:?}", packet.packet_size);
    assert!(packet.buf[..packet.packet_size] == dummy_ip_frame[..], "Expected {:?}, Got {:?}", dummy_ip_frame, packet.buf);
    handle.abort();
}   