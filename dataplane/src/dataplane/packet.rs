...
impl Packet {
    ...
    fn embed_route_id_to_ip_options(&mut self, route_id: usize) {
        ...
        if self.packet_size < 20 || self.buf[0] >> 4 != 4 {
            debug!("Not IPv4 packet, cannot embed route_id");
            return;
        }
        ...
        if new_header_len + (self.packet_size - current_header_len) > self.buf.len() {
            debug!("Not enough buffer space to embed route_id");
            return;
        }
        ...
        debug!("Embedded route_id {} into IP options", route_id);
    }
    ...
    fn extract_route_id_from_ip_options(&self) -> Option<usize> {
        ...
        if let Some(route_id) = self.find_route_id_option() {
            debug!("Extracted route_id {} from IP options", route_id);
        ...
        while option_offset < header_len {
            ...
            if option_type == 0x94 && option_length == 6 {
                ...
                if option_offset + 6 <= header_len {
                    ...
                    debug!("Updated route_id to {} in IP options", route_id);
                    return;
                }
            }
    ...
    fn get_flow_id_from_buf(buf: &PacketBuf, packet_size: usize) -> FlowId {
        ...
        if packet_size == 0 {
            error!("Zero-length packet received, returning zero flow_id");
            return 0;
        }

        if packet_size > buf.len() {
            error!("Packet size {} exceeds buffer length {}, using buffer length", 
                     packet_size, buf.len());
        }
        ...
        if packet_size < 20 || buf[0] >> 4 != 4 {
            ...
            debug!("Non-IPv4 packet detected - version: {}, size: {}", 
                     buf[0] >> 4, packet_size);
            ...
            debug!("Non-IPv4 packet, using fallback flow_id: {:#x}", old_flow_id as u128);
        ...
        debug!("Packet IP info - src_ip: {}.{}.{}.{}, dst_ip: {}.{}.{}.{}",
        ...
        if ihl < 5 || transport_header_start > packet_size {
            ...
            debug!("Invalid IHL ({}) or insufficient packet size, using zero flow_id", ihl);
            return 0;
        }
        ...
        let (src_port, dst_port) = if packet_size >= transport_header_start + 4 {
            match buf[9] {
                6 | 17 => {
                    ...
                    // Debug info
                    debug!("Protocol: {}, src_port: {}, dst_port: {}", 
                        if buf[9] == 6 { "TCP" } else { "UDP" }, src_port, dst_port);
                    (src_port, dst_port)
                }
                _ => {
                    // Protocol - no port extraction
                    debug!("Unknown protocol: {}, no ports extracted", buf[9]);
                    (0, 0) // Other protocols
                }
            }
        } else {
            // Insufficient data for transport header
            debug!("Insufficient data for transport header, no ports extracted");
            (0, 0) // Not enough data
        };
        ...
        debug!("Created flow_id: {:#x} from src_ip: {}, dst_ip: {}, src_port: {}, dst_port: {}", 
