...
def measure_links(conn, prev_read, n_nodes, verbose=False):
...
def measure_flows(conn, prev_read, n_nodes, verbose=False):
    ...
    for flow_id in flow_ids:
        if flow_id[2] != 0: #ignore flow_ids that belongs to multipath
            continue
    ...
    for flow_id in flow_ids:
        if flow_id[2] != 0: #ignore flow_ids that belongs to multipath        if flow_id[2] != 0: #ignore flow_ids that belongs to multipath
            continue 
    ...
    for metric in metrics:
        flow_id = metric[0]
        flow_id[2] = 0 #convert flow_ids that belongs to multipath
        flow_id[6] = 0 #convert flow_ids that belongs to multipath
