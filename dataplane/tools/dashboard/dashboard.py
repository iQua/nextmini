...
class Database:
    ...
    def display_db_pernode(self):
        ...
        cursor.execute('''
            SELECT dst_node_id, SUM(bps) AS total_bps
            FROM metrics
            WHERE (dst_node_id, time_read) IN (
                SELECT dst_node_id, MAX(time_read) 
                FROM metrics
                GROUP BY dst_node_id)
            GROUP BY dst_node_id
            ORDER BY dst_node_id ASC;
        ''')
        metrics = cursor.fetchall()
        
        self.print_buffer.append(str("╔══════════════════════════════════════════════════════════════╗"))
        self.print_buffer.append(str("║                    Total BPS Per Node                        ║"))
        self.print_buffer.append(str("╠══════════════════════════════════════════════════════════════╣"))
        self.print_buffer.append(str("║       Source Node ID      │        Total BPS (MiBits)        ║"))
        self.print_buffer.append(str("╠══════════════════════════════════════════════════════════════╣"))
        if len(metrics) == 0:
            self.print_buffer.append(str("║" + "No Data Available".center(62) + "║"))
        else:
            for metric in metrics:
                self.print_buffer.append(str(f"║ {metric[0]:^25} │ {metric[1]/1000000:^32} ║"))
        self.print_buffer.append(str("╚══════════════════════════════════════════════════════════════╝"))
    ...
    def display_db_perlink(self):
        ...
        self.print_buffer.append(str("╔══════════════════════════════════════════════════════════════╗"))
        self.print_buffer.append(str("║                      Total BPS Per Link                      ║"))
        self.print_buffer.append(str("╠══════════════════════════════════════════════════════════════╣"))
        self.print_buffer.append(str("║ Source Node ID │ Destination Node ID │   Total BPS(MiBits)   ║"))
        self.print_buffer.append(str("╟────────────────┼─────────────────────┼───────────────────────╢"))
        if len(metrics) == 0:
            self.print_buffer.append(str("║" + "No Data Available".center(62) + "║"))
        else: 
            for metric in metrics:
                self.print_buffer.append(str(f"║ {metric[0]:^14} │ {metric[1]:^19} │ {metric[2]/1000000:^21} ║"))
        self.print_buffer.append(str("╚══════════════════════════════════════════════════════════════╝"))
    ...
    def display_db_perflow(self):
        ...
        # Find all flow ids
        cursor.execute('''
            SELECT flow_id, src_node_id, dst_node_id, SUM(bps) AS total_bps
            FROM metrics
            WHERE (dst_node_id, time_read) IN (
                SELECT dst_node_id, MAX(time_read) 
                FROM metrics
                GROUP BY dst_node_id)
            GROUP BY flow_id, src_node_id, dst_node_id
        ''')
        data = [(arr[0], arr[1],arr[2],arr[3]) for arr in cursor.fetchall()]

        self.print_buffer.append(str("╔══════════════════════════════════════════════════════════════════════════════════════╗"))
        self.print_buffer.append(str("║                                              Total BPS Per Flow                      ║"))
        self.print_buffer.append(str("╠══════════════════════════════════════════════════════════════════════════════════════╣"))
        self.print_buffer.append(str("║              Flow ID                 │       Link ID         │   Total BPS (MiBits)  ║"))
        self.print_buffer.append(str("╟──────────────────────────────────────┼───────────────────────┼───────────────────────╢"))
        if len(data) == 0:
            self.print_buffer.append(str("║" + "No Data Available".center(62) + "║"))
        else: 
            for flow_id, src_node_id, dst_node_id, bps in data:
                flow_id = ".".join(map(str, flow_id[:4])) + " -> " + ".".join(map(str, flow_id[4:]))
                link = f"{src_node_id} -> {dst_node_id}"
                self.print_buffer.append(str(f"║ {flow_id:^37}│  {link:^20} | {bps/1000000:^21} ║"))
        self.print_buffer.append(str("╚══════════════════════════════════════════════════════════════════════════════════════╝"))   
    ...
    def display_historic_db_pernode(self,table: np.ndarray):
        self.print_buffer.append(str("╔══════════════════════════════════════════════════════════════╗"))
        self.print_buffer.append(str("║                    Total BPS Per Node                        ║"))
        self.print_buffer.append(str("╠══════════════════════════════════════════════════════════════╣"))
        self.print_buffer.append(str("║       Source Node ID      │        Total BPS (MiBits)        ║"))
        ...
        if len(table) == 0:
            ...
            for columns in table:
                for column in columns:
                    if column.all():
                        self.print_buffer.append(str(f"║ {column[1]:^25} │ {column[2]/1000000:^32} ║"))
        self.print_buffer.append(str("╚══════════════════════════════════════════════════════════════╝")) 

    def display_historic_db_perlink(self,table: np.ndarray):
        self.print_buffer.append(str("╔══════════════════════════════════════════════════════════════╗"))
        self.print_buffer.append(str("║                      Total BPS Per Link                      ║"))
        self.print_buffer.append(str("╠══════════════════════════════════════════════════════════════╣"))
        self.print_buffer.append(str("║ Source Node ID │ Destination Node ID │   Total BPS(MiBits)   ║"))
        ...
        if len(table) == 0:
            ...
            for columns in table:
                for column in columns:
                    if column.all():
                        self.print_buffer.append(str(f"║ {column[1]:^14} │ {column[2]:^19} │ {column[3]/1000000:^21} ║"))
        self.print_buffer.append(str("╚══════════════════════════════════════════════════════════════╝"))    

    def display_historic_db_perflow(self,table: np.ndarray):
        self.print_buffer.append(str("╔══════════════════════════════════════════════════════════════╗"))
        self.print_buffer.append(str("║                      Total BPS Per Flow                      ║"))
        self.print_buffer.append(str("╠══════════════════════════════════════════════════════════════╣"))
        self.print_buffer.append(str("║              Flow ID                 │   Total BPS (MiBits)  ║"))
        self.print_buffer.append(str("╟──────────────────────────────────────┼───────────────────────╢"))
        if len(table) == 0:
            self.print_buffer.append(str("║" + "No Data Available".center(62) + "║"))
        else: 
            for columns in table:
                for column in columns:
                    if column.all():
                        ...
                        self.print_buffer.append(str(f"║ {flow_id_display:^37}│ {column[2]/1000000:^21} ║"))
        self.print_buffer.append(str("╚══════════════════════════════════════════════════════════════╝"))        
