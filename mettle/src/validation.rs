...
fn small_stream_bin_parts(bins: impl IntoIterator<Item = MettleBin>) -> Vec<(u128, [u8; 2])> {
    bins.into_iter()
...
fn validation_harness_matches_small_stream_bin_goldens() {
    ...
    assert_eq!(small_stream_bin_parts(bins), EXPECTED_SMALL_STREAM_BINS);
}
...
fn validation_harness_matches_kept_bin_goldens_after_prefix_erasure() {
    ...
    let expected = EXPECTED_SMALL_STREAM_BINS
        .into_iter()
        .filter(|(bin_id, _)| *bin_id != params.tle_bin_id(0))
        .collect::<Vec<_>>();

    assert_eq!(
        small_stream_bin_parts(kept_bins),
        expected
    );
}
