use mettle::block::{BlockError, BlockParams, DecodeError, Decoder, Encoder};
...
fn decode_with_missing_sources(
    ...
    let mut symbols = (0..params.source_symbols)
        ...
        .map(|source_index| {
            decoder.source_symbol(
                source_index,
                encoder
                    .source_symbol(source_index)
                    .expect("source symbol exists"),
            )
        })
        .collect::<Vec<_>>();

    let metadata = encoder.metadata().expect("metadata");
...
#[test]
fn source_only_block_decode() {
    let params = BlockParams::new(32, 16, 0);
    let source_block = source_block(params.source_symbols, params.symbol_size);
    let encoder = Encoder::from_block(params, &source_block).expect("valid block encoder");
    let decoder = Decoder::from_block(params);

    let symbols = (0..params.source_symbols)
        .map(|source_index| {
            decoder.source_symbol(
                source_index,
                encoder
    ...
    let output = decoder.decode(&symbols).expect("source-only decode");
...
#[test]
fn duplicate_source_and_repair_insertion_is_harmless() {
    let params = BlockParams::new(64, 8, 17);
    let source_block = source_block(params.source_symbols, params.symbol_size);
    let encoder = Encoder::from_block(params, &source_block).expect("valid block encoder");
    let decoder = Decoder::from_block(params);
    let metadata = encoder.metadata().expect("metadata");
    ...
    for source_index in 0..params.source_symbols {
        ...
        let payload = encoder
            .source_symbol(source_index)
            .expect("source symbol exists");
        symbols.push(decoder.source_symbol(source_index, payload.clone()));
        symbols.push(decoder.source_symbol(source_index, payload));
    }
    ...
    for repair_index in 0..metadata.repair_symbol_count() {
        let payload = encoder
            .repair_symbol(repair_index)
            .expect("repair symbol exists");
        symbols.push(decoder.repair_symbol(repair_index, payload.clone()));
        symbols.push(decoder.repair_symbol(repair_index, payload));
    }

    let output = decoder
        .decode(&symbols)
        .expect("duplicate-heavy input decodes");
    assert_eq!(
        output.source_symbols,
        expected_symbols(&source_block, params.symbol_size)
    );
...
fn metadata_only_large_k_maps_repair_indexes_without_payload_pressure() {
    ...
    assert!(deficit.next_repair_index >= 18);
    assert!(deficit.decoded_source_symbols >= 4090);
}
...
fn fixed_k_padded_block_boundary_is_not_trimmed() {
    ...
    assert_eq!(output.into_source_block(), source_block);
}
...
#[test]
fn invalid_received_symbol_indexes_return_decode_errors() {
    let params = BlockParams::new(4, 4, 0);
    let decoder = Decoder::from_block(params);
    let metadata = params.metadata().expect("metadata");

    let bad_source = decoder.source_symbol(params.source_symbols, vec![0; params.symbol_size]);
    assert!(matches!(
        decoder.decode(&[bad_source]),
        Err(DecodeError::Block(BlockError::SourceIndexOutOfRange {
            source_index: 4,
            source_symbols: 4,
        }))
    ));
    ...
    let bad_repair =
        decoder.repair_symbol(metadata.repair_symbol_count(), vec![0; params.symbol_size]);
    assert!(matches!(
        decoder.decode(&[bad_repair]),
        Err(DecodeError::Block(BlockError::RepairIndexOutOfRange { .. }))
    ));
}
