...
We can see the local TUN interface, and it is assigned an IP address of `10.0.0.1`. By default, controller config uses `base_addr = 10.0.0.0` with `net_mask = 255.255.0.0` (`/16`) for the Nextmini dataplane address space. For a node with an ID _n_, its corresponding base IP address is `10.0.0.n`.
