...
pub(crate) async fn recompute_group_routes(
    ...
    let previous_snapshot = load_previous_edges(db_pool, group_id).await?;
    ...
    let previous_edges = previous_snapshot.edges;
    ...
    Ok(Some(RecomputedGroupRoutes {
        ...
        member_node_set,
        previous_edges,
        has_group_routes: previous_snapshot.has_group_routes,
        dag_edges,
        dag_nodes,
    }))
}
...
struct GroupRoutesSnapshot {
    edges: Vec<(u32, u32)>,
    has_group_routes: bool,
}

async fn load_previous_edges(
    db_pool: &Pool<Postgres>,
    group_id: i32,
) -> AnyResult<GroupRoutesSnapshot> {
