...
use crate::ObjectId;
use crate::deterministic::DetRng;
use crate::gf256::{Gf256, gf256_addmul_slice};
use crate::proof::{
    DecodeConfig, DecodeProof, EliminationTrace, FailureReason, PeelingTrace, ReceivedSummary,
};
use crate::systematic::{ConstraintMatrix, RobustSoliton, SystematicParams};
...
impl InactivationDecoder {
    ...
    fn peel(state: &mut DecoderState) {
        loop {
            ...
            if coef != Gf256::ONE {
                let inv = coef.inv();
                crate::gf256::gf256_mul_slice(&mut solution, inv);
            }
    ...
    fn inactivate_and_solve(&self, state: &mut DecoderState) -> Result<(), DecodeError> {
        ...
        for col in 0..n_cols {
            ...
            crate::gf256::gf256_mul_slice(&mut b[prow], inv);
...
mod tests {
    use super::*;
    use crate::systematic::SystematicEncoder;
