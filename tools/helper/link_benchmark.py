...
class LinkBenchmark:
    ...
    def benchmark_all_pairs(
        ...
        for node in nodes:
            print(f"  Node {node.id}: private={node.private_ip}, public={node.public_ip}")
    ...
    def get_throughput_matrix(self, protocol: str = "tcp") -> Dict[Tuple[int, int], float]:
        """
        ...
        return {
            (r["src_node_id"], r["dst_node_id"]): r["bandwidth_mbps"]
            for r in results
        }
...
def main():
    ...
    measure_parser.add_argument("-t", "--duration", type=int, default=10, help="Test duration per pair")
    measure_parser.add_argument("-u", "--udp", action="store_true", help="Use UDP")
    measure_parser.add_argument("--one-way", action="store_true", help="Only test A->B, not B->A")
    measure_parser.add_argument("--public", action="store_true", help="Use public addresses")
    measure_parser.add_argument("--parallel", action="store_true", help="Run tests in parallel")
    ...
    query_parser.add_argument("-u", "--udp", action="store_true", help="Filter by UDP protocol")
    query_parser.add_argument("--all", action="store_true", help="Show all measurements, not just latest")
    
    # Matrix command
    matrix_parser = subparsers.add_parser(
        "matrix", parents=[db_args], help="Show throughput matrix"
    )
    matrix_parser.add_argument("-u", "--udp", action="store_true", help="Show UDP matrix")
    ...
    server_parser.add_argument("-p", "--port", type=int, default=5201, help="Server port")
    ...
    try:
        if args.command == "nodes":
            ...
            if successful:
                ...
                print(f"Bandwidth: avg={avg_bw:.1f} Mbps, min={min_bw:.1f} Mbps, max={max_bw:.1f} Mbps")
            if failed:
            ...
            for r in results:
                print(f"  {r['src_node_id']} -> {r['dst_node_id']}: "
