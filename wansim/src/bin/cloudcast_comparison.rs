...
use wansim::experiment::{CloudcastComparisonArtifacts, run_cloudcast_comparison_persistent};

const USAGE: &str =
    "usage: cloudcast_comparison <seeds=16> <workers> <output-directory> [--resume]";

fn main() -> ExitCode {
    match run() {
        Ok(()) => ExitCode::SUCCESS,
        Err(error) => {
            eprintln!("cloudcast_comparison: {error}");
            ExitCode::FAILURE
        }
    }
}
...
fn write_artifacts(
    output: &Path,
    artifacts: &CloudcastComparisonArtifacts,
) -> Result<(), std::io::Error> {
