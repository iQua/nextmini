...
impl TestRoutingTable {
    ...
    pub fn get_available_routes_for_direction(&self, src: NodeId, dst: NodeId) -> Option<&Vec<usize>> {
...
fn main() {
    println!("=== Route Selection Test: Node 4 to Node 1 ===\n");
    
    // Create manual routes from config
    let manual_routes = vec![
        Route { route_id: 1, path: vec![1, 2, 3, 4] },
        Route { route_id: 2, path: vec![1, 3, 2, 4] },
        Route { route_id: 3, path: vec![1, 3, 4] },
        Route { route_id: 4, path: vec![4, 3, 2, 1] },  // 4->1 route 1
        Route { route_id: 5, path: vec![4, 2, 3, 1] },  // 4->1 route 2
        Route { route_id: 6, path: vec![4, 3, 1] },     // 4->1 route 3
        Route { route_id: 7, path: vec![1, 2] },
        Route { route_id: 8, path: vec![2, 1] },
    ];
    ...
    let mut route_4_to_1: Vec<Route> = all_routes.iter()
    ...
    println!("Manual routes (from config):");
    for route in &manual_routes {
        println!("  Route {}: {:?}", route.route_id, route.path);
    }
    
    // Show all 4->1 routes specifically since that's what we're testing
    let routes_4_to_1: Vec<_> = all_routes.iter()
        .filter(|r| r.path.len() >= 2 && r.path[0] == 4 && r.path[r.path.len()-1] == 1)
        .collect();
    
    println!("\nAll routes from Node 4 to Node 1 (renumbered sequentially):");
    for route in &routes_4_to_1 {
        println!("  Route {}: {:?} ({} hops)", route.route_id, route.path, route.path.len() - 1);
    }
    
    println!("\nOther full mesh routes (sample):");
    for route in all_routes.iter().filter(|r| !(r.path.len() >= 2 && r.path[0] == 4 && r.path[r.path.len()-1] == 1)).take(5) {
        println!("  Route {}: {:?}", route.route_id, route.path);
    }
    println!("  ... (total {} routes)\n", all_routes.len());
    
    // Test Node 4's routing table
    println!("=== Testing Node 4 Routing Table ===");
    let node4_routes = build_routes_for_node(all_routes.clone(), 4);
    
    println!("Node 4 can handle {} routes:", node4_routes.len());
    ...
    if let Some(available_routes) = node4_table.get_available_routes_for_direction(4, 1) {
        println!("Found {} routes for 4→1:", available_routes.len());
        ...
        for &route_id in available_routes {
            ...
            if let Some(original_route) = all_routes.iter().find(|r| r.route_id == route_id) {
                println!("  Route {}: path {:?} -> next_hop {}", 
                    route_id, original_route.path, next_hop);
            }
        }
    } else {
        println!("No routes found for direction 4→1!");
        return;
    }
    ...
    println!("Distribution across {} flows:", num_flows);
    ...
    for flow_id in test_flows {
        ...
        println!("  Flow {:#018x}: route {} -> next_hop {} (consistent: {})", 
    ...
    println!("  Slow path (new flow): {:?}", slow_path_time);
    println!("  Fast path (1000 cached lookups): {:?}", fast_path_time);
    println!("  Average fast path: {:?}", fast_path_time / 1000);
    
    println!("\n=== Test Summary ===");
    println!("✓ Successfully loaded {} total routes", all_routes.len());
    println!("✓ Node 4 has {} applicable routes", node4_table.route_next_hop.len());
    if let Some(routes_4_to_1) = node4_table.get_available_routes_for_direction(4, 1) {
        println!("✓ Found {} routes for 4→1 direction", routes_4_to_1.len());
        println!("✓ Jump hash provides even distribution across all routes");
        println!("✓ Flow routing is consistent across multiple lookups");
        println!("✓ Cached lookups are ~{}x faster than initial lookup", 
