"""
Training script adapted from Lambda Labs distributed-training-guide
Original: https://github.com/LambdaLabsML/distributed-training-guide
License: MIT (see license header above)
Modifications: Adapted for CPU training and overlay network testing
"""
...
def main():
    ...
    # Standard pytorch dataset iterator (use DistributedSampler when distributed)
    if distributed:
    ...
    # Derive a default experiment name if none provided
    if args.experiment_name is None:
        import datetime

        timestamp = datetime.datetime.now().strftime("%Y-%m-%dT%H-%M-%S")
        args.experiment_name = f"{args.model_name.replace('/', '-')}-{args.dataset_name.replace('/', '-')}-{timestamp}"
    ...
    # Wrap in DistributedDataParallel after (possibly) loading a checkpoint
    if distributed:
