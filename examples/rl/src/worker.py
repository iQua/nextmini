...
class Worker:
    def __init__(self, rank: int, device_id: int = 0, config_path: str = None):
        ...
        self.model = AutoModelForCausalLM.from_pretrained(
            config.MODEL_NAME, dtype=torch.float16, trust_remote_code=True
        ).to(self.device)
        self.model.eval()
    ...
    async def run(self):
        ...
        while True:
            ...
            if msg["type"] == "HANDSHAKE_ACK":
                ...
                print(
                    f"Worker {self.rank} received HANDSHAKE_ACK. Trainer user IP: {self.trainer_user_ip}",
                    flush=True,
                )
                continue
            ...
            if msg["type"] == "WEIGHT_METADATA":
                ...
                print(
                    f"Worker {self.rank} received weight metadata. Preparing for Multicast sync...",
                    flush=True,
                )
                ...
                if not isinstance(session_id, int) or session_id <= 0:
                    print(
                        f"Worker {self.rank}: invalid or missing session_id in WEIGHT_METADATA: {msg}",
                        flush=True,
                    )
                    continue

                print(
                    f"Worker {self.rank} metadata: group_id={group_id}, group_ip={group_ip}, size={size} bytes, session_id={session_id}",
                    flush=True,
                )

                # Join the multicast group
                print(
                    f"Worker {self.rank} joining multicast group {group_id}...",
                    flush=True,
                )
                ...
                try:
                    ...
                    print(
                        f"Worker {self.rank} receive_data session established. SID={sid}",
                        flush=True,
                    )
                except Exception as e:
                    print(
                        f"Worker {self.rank} failed to establish session: {e}",
                        flush=True,
                    )
                    continue
                ...
                ok = await self.dataplane.reliable_wait_async(
                ...
                for prompt in prompts:
                    ...
                    results.append({"prompt": prompt, "completions": texts})
                ...
                if not self.trainer_user_ip:
                    print(
                        f"Worker {self.rank}: trainer_user_ip not set, cannot send reliable rollout.",
                        flush=True,
                    )
                    continue
                ...
                try:
                    ...
                    print(
                        f"Worker {self.rank}: error sending reliable rollout data: {e}",
                        flush=True,
                    )
                    continue
