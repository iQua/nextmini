...
def archive_last_run_into(run_dir: pathlib.Path, *, label: str) -> None:
    """Archive the most recent run outputs (results + meta + manifest) into run_dir."""
    _ensure_dir(run_dir)

    for name in ("results.json", "results.meta.json", "manifest.json"):
    ...
    if manifest.exists():
        try:
            ...
            pass
    meta = RESULTS_DIR / "results.meta.json"
    if meta.exists():
        try:
            m = _read_json(meta)
            idx["tree_max_depth"] = m.get("tree_max_depth")
            idx["used_nonterminal_relays"] = m.get("used_nonterminal_relays")
            idx["used_worker_forwarders"] = m.get("used_worker_forwarders")
            idx["probe_stats"] = m.get("probe_stats")
        except Exception:
            pass
...
def run_bench(
    *,
    ...
    if rc != 0:
        raise SystemExit(f"run-bench failed (label={label}). See {run_dir}/stdout.txt")
...
def main() -> int:
    p = argparse.ArgumentParser(description="Run WAN matrix and archive each run's outputs.")
    p.add_argument("--inventory", type=pathlib.Path, required=True)
    p.add_argument("--batch-ssh", action="store_true")
    p.add_argument("--out-dir", type=pathlib.Path, default=REPO_ROOT / "examples" / "rl" / "multidc" / "archives")
    p.add_argument("--label", type=str, default="manual")
    p.add_argument(
        "--args",
        type=str,
        default="",
        help="Extra args to pass to multidc.py run-bench (as a single shell-like string).",
    )
