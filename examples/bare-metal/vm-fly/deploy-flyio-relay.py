...
def get_app_ipv4(app_name):
    ...
    try:
        ...
        for ip_info in ips:
            ...
            ip_addr = ip_info.get("Address") or ip_info.get("address") or ip_info.get("IP")
...
def create_node_config(template_path, output_path, controller_ip, node_id, public_ipv4=None):
    """Create node config from template."""
    # Set both private and public network addr to the Fly.io allocated IPv4
    addr_config = f'private_network_addr = "{public_ipv4}"\npublic_network_addr = "{public_ipv4}"' if public_ipv4 else ""
