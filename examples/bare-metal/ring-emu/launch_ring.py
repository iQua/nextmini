...
async def scp_to_host(
    local: pathlib.Path, host: str, remote_path: str, ssh_port: int, ssh_key=None
):
...
async def main():
    ...
    if not args.no_copy:
        ...
        for host in ssh_targets:
            tasks.append(
                scp_to_host(
                    args.ring, host, remote_ring_path, args.ssh_port, args.ssh_key
                )
            )
        await asyncio.gather(*tasks)
        ...
        for host in ssh_targets:
            tasks.append(
                scp_to_host(
                    args.bin, host, remote_bin_path, args.ssh_port, args.ssh_key
                )
            )
        await asyncio.gather(*tasks)
