...
def parse_ring_hosts(ring_lines: List[str]) -> List[Tuple[str, int]]:
    ...
    for i, line in enumerate(ring_lines):
        if ":" not in line:
            raise SystemExit(f"ERROR: ring line {i+1!r} must be IP:port, got {line!r}")
...
def derive_ssh_targets(ring_hosts: List[Tuple[str, int]],
                       ssh_hosts_file: pathlib.Path | None) -> List[str]:
    if ssh_hosts_file:
...
async def run_cmd(cmd: List[str], *, stdin=None, capture=False, pipe_stderr=False):
    proc = await asyncio.create_subprocess_exec(
        *cmd,
        ...
        stderr=asyncio.subprocess.STDOUT if pipe_stderr else (asyncio.subprocess.PIPE if capture else None),
    )
    if capture:
...
async def main():
    ...
    ap.add_argument("--ring", type=pathlib.Path, required=True,
    ...
    ap.add_argument("--remote-dir", type=str, default="~/ringallreduce_run",
                    help="Remote working directory on each host (default: %(default)s).")
    ap.add_argument("--ssh-hosts", type=pathlib.Path, default=None,
    ...
    ap.add_argument("--remote-ring-name", type=str, default="ring.txt",
                    help="Filename to use for the ring file on the remote side.")
    ap.add_argument("--strict-host-key-checking", action="store_true",
