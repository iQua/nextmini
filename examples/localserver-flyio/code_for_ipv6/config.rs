...
impl LocalConfig {
    ...
    pub fn new() -> LocalConfig {
        ...
        // sets the private network address of the network interface
        // Supports both IPv4 and IPv6 addresses
        // For IPv6, prioritizes Fly.io private network addresses (fdaa: prefix)
        ...
        if cfgs.private_network_addr.is_empty() {
                    ...
                    for addr in itf.addr.iter() {
                        match addr {
                            Addr::V6(ipv6) => {
                                ...
                                // Prioritize Fly.io private network IPv6 (fdaa: prefix)
                                // or any ULA (Unique Local Address, fc00::/7)
            ...
            // Only compute node_id from IPv4 address if node_id is not configured
            if cfgs.node_id == 0 {
                if let Ok(real_ip) = cfgs.private_network_addr.parse::<Ipv4Addr>() {
                    ...
                    // IPv6 or non-parseable address - node_id must be configured
                    if cfgs.private_network_addr.is_empty() {
                        error!(
        ...
        // sets the public network address of the network interface
        // Supports both IPv4 and IPv6 addresses
                ...
                if itf.name == itf_name {
                    ...
                    for addr in itf.addr.iter() {
                        match addr {
                            Addr::V6(ipv6) => {
                                let ipv6_str = ipv6.ip.to_string();
                                // Prioritize Fly.io private network IPv6 or ULA
                                if ipv6_str.starts_with("fdaa:") || ipv6_str.starts_with("fd") {
                                    if ipv6addr.is_empty() || ipv6_str.starts_with("fdaa:") {
                                        ipv6addr = ipv6_str;
                                    }
