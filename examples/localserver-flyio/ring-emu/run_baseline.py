...
async def cleanup_node(app_name: str):
    ...
    cmd = [
        ...
        app_name,
        "-C",
        "killall -9 ringallreduce 2>/dev/null || true",
    ]
...
async def trigger_node(
    app_name: str, rank: int, length: int, reps: int, init: str, verify: bool
):
    ...
    cmd = ["flyctl", "ssh", "console", "-a", app_name, "-C", ringallreduce_cmd]
...
async def main():
    ...
    parser.add_argument(
        "--init", type=str, default="ones", choices=["rank", "ones", "random"]
    )
