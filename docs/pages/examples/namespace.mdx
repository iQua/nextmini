...
To prevent overwhelming the controller and the PostgreSQL database when spawning a large number of nodes, Nextmini supports connection staggering:
...
This delay occurs in the parent process after completing critical operations for each node, like creating the veth pair, spawning the child process, and moving the veth peer into the child's network namespace.
...
Each node waits `idx × 200ms` before connecting to the controller.
...
![Disable Port Security on Arbutus Instance](`/images/examples/ns-public/arbutus-port-security.png`)
