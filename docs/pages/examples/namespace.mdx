...
To build and run this example with a specific number of nodes in a virtual or physical machine, you may use command-line arguments to specify the number of nodes:
...
Alternatively, specify the number of nodes in the configuration file, such as `examples/namespace/config.toml`:
...
To see detailed logs, set the `RUST_LOG` environment variable to `info`:
