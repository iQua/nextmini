...
The example in `examples/simple-routes` shows how the network topology and routes can be defined in the controller's configuration file.

To build and run this example in the `examples/simple-routes` directory:
...
Nextmini's controller uses the network topology to determine which neighbouring node a node should connect with and it joins. The dataplane only establishes TCP or QUIC connections between neighbouring nodes in the network topology.
