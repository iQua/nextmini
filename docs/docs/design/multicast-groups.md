...
- **Processor** (`dataplane/src/node/processor.rs`) calls `get_next_hops_by_flow`. When multiple hops exist, it clones the packet payload and enqueues each hop on the scheduler. Flow-to-route caching accelerates steady-state traffic but is cleared whenever new routes arrive.
- **Connector Max mode** (`dataplane/src/node/connector.rs`) now shares the same helper, maintaining per-next-hop TCP handles so replication reuses existing connections.
...
For questions or planned changes, coordinate via the `DOCS-PLAN` agent mail thread.
