## Last Updated with : Master Branch | Commits 623dd43

### General Instructions & To-Dos

1. Set up 2 linux instances in Arbutus, 1 with floating IP.
2. Access the local instance with proxy jumps in ssh in `code editor`
3. Install Docker in VMs.
4. Config IP and ports accoding to following guide (pending changes).
5. Test connectivity with iperf3.
6. (Pending) How to change the configuration to add more nodes? (how to change `./controller/config.json` file?)
7. Run deepspeed with strato accoding to following guide (pending changes).
