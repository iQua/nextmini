## A Simple Example Running `iperf3` in Docker Containers

Though Nextmini runs natively on Linux, the easiest way to get started with Nextmini is to run it within Docker containers. The Docker image is built atop the latest distribution of Alpine Linux and contains all the necessary dependencies. To run Nextmini on a single machine over multiple containers, you will need a Docker runtime for running Docker. On macOS, [OrbStack](https://orbstack.dev/) is strongly recommended as it offers a simpler and more performant setup than Docker Desktop.
