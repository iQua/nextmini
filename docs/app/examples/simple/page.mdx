## Several Simple Examples as a Starting Point

Though _Nextmini_ runs natively on Linux, the easiest way to get started with _Nextmini_ is to run it within Docker containers. The Docker image is built atop the latest distribution of Alpine Linux and contains all the necessary dependencies to run _Nextmini_.

### Docker Single Machine Setup: a Simple Example with `iperf3`

To run _Nextmini_ on a single machine over multiple containers, you will need a Docker runtime for running Docker. On macOS, [OrbStack](https://orbstack.dev/) is strongly recommended as it offers a simpler and more performant setup than Docker Desktop.
