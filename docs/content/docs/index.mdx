...
```mermaid
flowchart LR
  subgraph Repo[Nextmini Repository]
    direction TB
    M[messages crate]
    C[controller crate]
    D[dataplane crate]
    P[python-api crate]
    G[cert-gen crate]
    T[tools / examples / docs]
  end

  W[Workloads and applications] -->|TUN-backed virtual network| D
  C -->|topology, routes, flow policy| D
  D -->|flow metrics and status| C
  D -->|inter-node transport tcp/udp/quic| DP[peer dataplanes]
  P -->|embedded dataplane APIs| D

  M --> C
  M --> D
  M --> P

  T -->|orchestration and experiments| C
  T -->|orchestration and experiments| D
  G -->|TLS and certificates| C
  G -->|TLS and certificates| D
```
