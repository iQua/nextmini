...
The following diagram shows the high-level structure of the repository and how the major runtime pieces interact.

<figure className="my-6">
  <img
    src="/images/birds-eye-view.svg"
    alt="Nextmini architecture overview: application workloads, control plane, dataplane runtime, and shared protocol"
    className="block w-full"
  />
  <figcaption className="mt-2 text-center text-sm text-fd-muted-foreground">
    Bird's-eye view of Nextmini's architectural design.
  </figcaption>
</figure>

<figure className="my-6">
  <img
    src="/images/dataplane-firstcut.svg"
    alt="Nextmini dataplane: Controller/Network/Local/UserSpaceFlows interfaces feed Processor + routing table, Scheduler, and Network Interface to next hop"
    className="block w-full"
  />
  <figcaption className="mt-2 text-center text-sm text-fd-muted-foreground">
    Designing Nextmini: a first-cut starting point — interfaces feed processor tasks (with routing tables) into the scheduler and out to the next hop.
  </figcaption>
</figure>
