...
Application APIs and concrete call examples are documented in [`Python API`](/docs/python-api). Implementation notes for the Python embedding path are in [`Python API Design`](/docs/design/python-api-design). This page focuses on control-plane and dataplane mechanics.
